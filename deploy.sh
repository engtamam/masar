...
full_deploy() {
    ...
    log "║                                                         ║"
    log "║  Admin account (CHANGE PASSWORD after first login!):    ║"
    log "║    Email:    $(grep '^ADMIN_EMAIL=' .env | cut -d= -f2 || echo 'admin@masar.sa')             ║"
    log "║    Password: (see your .env ADMIN_PASSWORD)             ║"
    log "║                                                         ║"
    log "║  Consultants → Created by admin from the admin panel    ║"
    log "║  Entrepreneurs → Register themselves from landing page  ║"
    log "║                                                         ║"
    log "║  Useful commands:                                       ║"
